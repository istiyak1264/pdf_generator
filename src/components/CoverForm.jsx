...
function calcProgress(values) {
  const required = ["courseTitle","courseCode","assignmentTopic","studentName","studentRoll","studentRegistration","session","teacherName"];
  ...
  return (
    <aside className={styles.panel}>
      ...
      <div className={styles.scrollArea}>
        ...
        <Section icon={Icons.Book} title="Course Information" subtitle="Title, code and topic" defaultOpen>
          <FormField label="Course Title" required error={errors.courseTitle}>
            <Input placeholder="e.g. Compiler Design" value={values.courseTitle} onChange={handle("courseTitle")} error={errors.courseTitle} />
          </FormField>
          <div className={styles.row}>
            <FormField label="Course Code" required error={errors.courseCode}>
              <Input placeholder="e.g. CSE 3103" value={values.courseCode} onChange={handle("courseCode")} error={errors.courseCode} />
            </FormField>
            <FormField label="Submission Date">
              <Input placeholder="DD.MM.YYYY" value={values.dateOfSubmission} onChange={handle("dateOfSubmission")} />
            </FormField>
          </div>
          <FormField label="Assignment Topic" required error={errors.assignmentTopic}>
            <Input placeholder="e.g. DFA, NFA, Regex, CFG and CNF" value={values.assignmentTopic} onChange={handle("assignmentTopic")} error={errors.assignmentTopic} />
          </FormField>
        </Section>
        ...
        <Section icon={Icons.User} title="Submitted By" subtitle="Student identity and class" defaultOpen>
          <FormField label="Student Name" required error={errors.studentName}>
            <Input placeholder="e.g. Md. Istiyak Ahmed" value={values.studentName} onChange={handle("studentName")} error={errors.studentName} />
          </FormField>
          <div className={styles.row}>
            <FormField label="Roll No." required error={errors.studentRoll}>
              <Input placeholder="230103" value={values.studentRoll} onChange={handle("studentRoll")} error={errors.studentRoll} />
            </FormField>
            <FormField label="Registration" required error={errors.studentRegistration}>
              <Input placeholder="1011988" value={values.studentRegistration} onChange={handle("studentRegistration")} error={errors.studentRegistration} />
            </FormField>
          </div>
          <FormField label="Session" required error={errors.session}>
            <Input placeholder="e.g. 2022-2023" value={values.session} onChange={handle("session")} error={errors.session} />
          </FormField>
          <div className={styles.row}>
        ...
        <Section icon={Icons.GradCap} title="Submitted To" subtitle="Teacher and department" defaultOpen>
          <FormField label="Teacher's Name" required error={errors.teacherName}>
            <Input placeholder="e.g. Nitun Kumar Podder" value={values.teacherName} onChange={handle("teacherName")} error={errors.teacherName} />
          </FormField>
      ...
      <div className={styles.actions}>
        <button className={styles.btnReset} onClick={onReset} type="button">Reset</button>
        <button
          className={`${styles.btnGenerate} ${generating ? styles.btnGenerating : ""}`}
          onClick={onGenerate} type="button" disabled={generating}
        >
          {generating ? (
